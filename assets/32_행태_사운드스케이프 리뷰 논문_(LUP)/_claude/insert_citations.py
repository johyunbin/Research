...
#   r2 2026-08-16: §2.6 보고편향 문턱 근거 — k ≥ 10 제한이 통상 지침(Sterne 2011)임을 명시.
#      Fable 전체 재검토에서 "검정 불가능" 표현을 "등록 문턱 미달로 미수행"으로 바로잡으면서
#      문턱 자체의 근거 인용을 요구하게 됐기 때문.
INSERTS = [
    ("따라서 어느 클러스터에서도 검정을 수행하지 않았다. 소규모 연구 효과(small-study "
     "effects)는 **탐지할 수단이 없으므로 배제할 수도 없으며**, 합성 추정치는 상한으로 "
     "읽어야 한다.",
     "이 문턱은 소수의 연구에서는 비대칭 검정의 검정력이 낮아 권장하지 않는 통상 지침을 "
     "따른 것이다{{sterne2011}}. 따라서 어느 클러스터에서도 검정을 수행하지 않았다. "
     "소규모 연구 효과(small-study effects)는 **탐지할 수단이 없으므로 배제할 수도 없다.**"),
]

# ── 신규 배경 문헌 ───────────────────────────────────────────────────
# key → DOI. **전건 Crossref 실재 확인을 통과한 것만 여기 넣는다.**
# 확인일 2026-08-16 · funnel plot 비대칭 검정 권고(BMJ)
NEW_REFS = {
    "sterne2011": "10.1136/bmj.d4002",
}
